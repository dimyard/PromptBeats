...
- `sound` выбирает kit: `lofi_kit`, `house_kit`, `trap_kit`, `boom_bap_kit`, `techno_kit`.
...
- synth step-grid → тоже меняет `track.events`: UI хранит выбранную ноту дорожки и переключает событие с парой
  `(step, note)`. Если такое событие есть — удаляет его; если нет — добавляет `{ step, note, dur: 1, vel: 0.8 }`.
  События с другими нотами на том же step сохраняются, чтобы работали аккорды;
- для sampler выбранная нота берётся из Drum note map; для synth UI даёт выбор ноты. Дефолты для новой/пустой дорожки:
  `bass → C2`, `chords/pad → C3`, `lead/fx → C4`.
...
Если во время `load(song)` Player уже играет и `bars` не меняется, он сохраняет текущую позицию Transport: события,
которые ещё впереди плейхеда, применяются в этом лупе; уже пройденные события — на следующем. `bpm`, `gain`, `muted`
и `sound` тоже применяются без restart. Изменение `bars` — структурная правка лупа: Player безопасно начинает новый
луп с шага `0`.
