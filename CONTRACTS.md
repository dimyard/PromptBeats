...
### Sampler engine и ручные контролы дорожек
...
- `gain`, `muted`, `sound` и `events` применяются при каждом `load(song)`, чтобы ручные контролы фронта работали через
  правку Song JSON без отдельного Player API.
