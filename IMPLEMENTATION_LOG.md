...
### 2026-07-11 · Забавные demo prompt chips · A / UX-Front
- **Что сделано:** расширены подсказки пользовательского ввода: добавлены пять шуточных демо-промптов для генерации
  треков, чтобы первый экран лучше подталкивал к экспериментам.
- **Где:** `frontend/src/App.jsx` (`DEMO_PROMPTS`).
- **Публичный интерфейс:** без изменений; Song JSON, HTTP API и Player API не менялись.
- **Как использовать:** `cd frontend && npm run dev`, затем нажать любой prompt chip в левой панели чата.
...
### 2026-07-11 · Расширенный каталог инструментов · C / интеграция A-B
- **Что сделано:** добавлены четыре синта (`warm_keys`, `acid_bass`, `organ`, `wide_pad`) и два синтезированных
  drum-кита (`boom_bap_kit`, `techno_kit`). Каталог синхронизирован со схемой, импортом, UI fallback и Player;
  LLM получает новые значения и краткие описания тембров в системном промпте.
- **Где:** `backend/src/catalog.js`, `backend/src/llm.js`, `song.schema.json`, `frontend/src/player/sounds.js`,
  `frontend/src/song-io.js`, `frontend/src/songEditing.js`, `CONTRACTS.md` и тесты frontend/backend.
- **Публичный интерфейс:** форма `Song` и `/api/catalog` не менялись; расширены допустимые значения `track.sound`.
- **Как использовать:** передать новый `sound` в совместимой паре `instrument` (`synth` или `sampler`) или запросить
  его в промпте LLM; Player соберёт соответствующий Tone.js voice без сетевых ассетов.
- **Изменение контракта:** расширен enum `track.sound` в `song.schema.json` и каталоге Contract.

### 2026-07-11 · Единое имя cymbal-пэда `C#3` · C
- **Что сделано:** устранено расхождение Drum note map: нота `C#3` теперь в контракте и Player называется `ride`.
  Синтезированный cymbal-voice и параметры kit-профилей переименованы соответственно; звучание не менялось.
- **Где:** `frontend/src/player/sounds.js`, `CONTRACTS.md`.
- **Публичный интерфейс:** без изменений: sampler-событие `{ step, note: "C#3" }` продолжает воспроизводиться.
- **Как использовать:** UI и Song JSON используют для `C#3` подпись `Ride`.
- **Отклонения от контракта:** нет; снята прежняя двусмысленность `ride/crash`.
...
- **Что сделано:** ручные изменения только BPM/gain/mute теперь обновляют существующие gain-ноды без пересборки
  аудиографа, поэтому звук не замирает при движении level. Scheduler события `step` создаётся один раз до `dispose()`,
  что предотвращает потерю визуального playhead после серии редактирований.
- **Где:** `frontend/src/player/index.js`, `frontend/src/player/MANUAL_SMOKE_CHECK.md`, `CONTRACTS.md`, briefs A/C.
- **Публичный интерфейс:** без изменений: UI продолжает вызывать `player.load(updatedSong)`.
- **Как использовать:** gain/mute/BPM можно менять подряд во время playback; для events/sound Player пересобирает
  граф без сброса позиции при неизменном Bars.
- **Отклонения от контракта:** нет; уточнены гарантии существующего `load(song)`.
...
- **Что сделано:** добавлена ручка drag-and-drop для перестановки дорожек, подсветка drop-target и прямой ввод BPM
  ...
  горизонтально на desktop/mobile, чтобы длинные лупы можно было просматривать без сжатия строк.
- **Где:** `frontend/src/App.jsx`, `frontend/src/styles.css`, `frontend/src/songEditing.js`,
  `frontend/test/songEditing.test.mjs`.
- **Публичный интерфейс:** новый helper `moveTrack(song, fromTrackId, toTrackId)`; Song JSON формат не менялся.
- **Как использовать:** тянуть `⋮⋮` у дорожки на другую строку; BPM можно ввести вручную и подтвердить `Enter` или blur;
  длинную дорожку прокручивать горизонтально внутри lane.
- **Отклонения от контракта:** нет.
- **Известные баги / TODO:** touch-friendly reorder пока через browser drag events; на мобильных может потребоваться
  отдельный режим стрелками/меню.

### 2026-07-11 · Fix: верхняя сетка и сворачиваемые сайдбары · A / UX-Front
- **Что сделано:** выровнены верхние блоки chat/studio/inspector по общей высоте, добавлены кнопки сворачивания
  левого чата и правого инспектора в узкие rail-колонки без пересчёта центральной студии.
- **Где:** `frontend/src/App.jsx`, `frontend/src/styles.css`.
- **Публичный интерфейс:** без изменений.
- **Как использовать:** кнопка `‹` в левом header скрывает чат, кнопка `›` в правом header скрывает инспектор;
  повторное нажатие возвращает панель.
- **Отклонения от контракта:** нет.
- **Известные баги / TODO:** настройки эффектов для дорожек/master требуют отдельного расширения Song JSON контракта.
### 2026-07-11 · Non-disruptive reload ручных правок · C
- **Что сделано:** `player.load(updatedSong)` во время playback больше не останавливает Transport при неизменном
  `bars`. Новые события впереди плейхеда звучат в текущем лупе, прошедшие — в следующем; BPM, gain, mute и sound
  применяются без restart. Изменение Bars остаётся структурной правкой и начинает луп с шага 0.
- **Где:** `frontend/src/player/index.js`, `frontend/src/player/MANUAL_SMOKE_CHECK.md`, `CONTRACTS.md`, briefs A/C.
- **Публичный интерфейс:** без изменений: UI продолжает вызывать `player.load(updatedSong)`.
- **Как использовать:** при ручной правке A не вызывает отдельно `stop()` и `play()`; просто передаёт полный Song в `load`.
- **Отклонения от контракта:** нет; уточнена гарантия существующего `load(song)`.
- **Проверено:** `node --check`, `npm test` (30/30), `npm run build`; smoke-check дополнен будущим synth-step.
- **Известные баги / TODO:** runtime-проверка в браузере остаётся обязательной, особенно для Tone.Part на границе лупа.
